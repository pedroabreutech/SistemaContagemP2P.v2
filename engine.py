...
"""
Funções de treino e avaliação usadas em main.py.
Em grande parte copiadas/adaptadas do DETR (https://github.com/facebookresearch/detr).
"""
import math
import os
import sys
...
def vis(samples, targets, pred, vis_dir, des=None):
    ...
    for idx in range(samples.shape[0]):
        ...
        size = 2
        # desenha a verdade de referência (ground truth)
        for t in gts[idx]:
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in data_loader:
        ...
        # etapa de propagação direta (forward)
        ...
        if not math.isfinite(loss_value):
            ...
            sys.exit(1)
        # etapa de retropropagação (backward)
        optimizer.zero_grad()
        losses.backward()
        if max_norm > 0:
    ...
    # reúne as estatísticas de todos os processos
...
def evaluate_crowd_no_overlap(model, data_loader, device, vis_dir=None):
    ...
    # executa inferência em todas as imagens para calcular MAE
    maes = []
    mses = []
    for samples, targets in data_loader:
        samples = samples.to(device)
        ...
        # 0.5 é usado por padrão
        threshold = 0.5

        points = outputs_points[outputs_scores > threshold].detach().cpu().numpy().tolist()
        predict_cnt = int((outputs_scores > threshold).sum())
        # se especificado, salva as imagens visualizadas
